...
public class SidePanel extends JPanel {
	...
	private static final Color DRAW_COLOR = new Color(100, 120, 140);
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString("Nivo: " + tetris.getLevel(), LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Rezultat: " + tetris.getScore(), LARGE_INSET, offset += TEXT_STRIDE);
		
		
		g.setFont(LARGE_FONT);
		g.drawString("Kontrole:", SMALL_INSET, offset = CONTROLS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Left arrow - Levo", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Right arrow - Desno", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Down arrow - Dole", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("R - Rotiranje", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("P - Pauza", LARGE_INSET, offset += TEXT_STRIDE);
		
		
		g.setFont(LARGE_FONT);
		g.drawString("Sledece", SMALL_INSET, 70);
